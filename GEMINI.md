...
* 인터랙티브 컴포넌트 구현 원칙: Svelte 컴포넌트의 클라이언트 측 하이드레이션 불안정성을 고려하여, 버튼 클릭 등 간단한 상호작용은 Astro 페이지 내의 일반 `<script>` 태그와 DOM 이벤트를 통해 구현하는 것을 우선적으로 고려한다. 복잡한 상태 관리가 필요한 경우에 한해 Svelte 컴포넌트 사용을 검토하되, 외부 UI 라이브러리 의존성을 최소화한다.
